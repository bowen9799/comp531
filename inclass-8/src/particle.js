...
var mod = function (a, b) {
    // used to calculate particles' position always in canvas
	return ((a % b) + b) % b
}

const update = ({acceleration, velocity, position, mass}, delta, canvas) => {
    position[0] = mod(position[0] + velocity[0] * delta, canvas.width)
    position[1] = mod(position[1] + velocity[1] * delta, canvas.height)
