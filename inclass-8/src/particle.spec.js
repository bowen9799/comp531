...
describe('Particle Functionality', () => {
    ...
    it('should update the velocity by the acceleration', () => {
        ...
        const p = particle({ position: [1, 1], velocity: [0.5, -0.5], acceleration: [ 1, 0.5 ] })
    ...
    it('particles should wrap around the world', () => {
        ...
        // canvas = { width, height }
        const p = particle({ position: [5, 5], velocity: [0.5, 1] })

        const p_l = update(p, -6, canvas)
        expect(p_l.position[0]).to.eql(5 - 6 * 0.5 + canvas.width)

        const p_r = update(p, 6, canvas)
        expect(p_r.position[0]).to.eql(5 + 6 * 0.5 - canvas.width)

        const p_t = update(p, 10, canvas)
        expect(p_t.position[1]).to.eql(5 + 10 - canvas.height)

        const p_b = update(p, -10, canvas)
        expect(p_b.position[1]).to.eql(5 - 10 + canvas.height)
    })
