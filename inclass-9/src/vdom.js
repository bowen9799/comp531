...
function updateElement(parent, newNode, oldNode, index=0) {
    ...
    if (!oldNode) {
        ...
        if (!newNode) {
            ...
            // console.log('change!')
            // remove the oldNode's DOM from parent
            parent.removeChild(parent.childNodes[index])
            // append the newNode's DOM to parent
            parent.appendChild(createElement(newNode))
        } else if (newNode.tag) {
            // modify  
            // console.log(newNode.children.length, oldNode.children.length)
